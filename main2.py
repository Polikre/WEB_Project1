...
name = ""
spis_facult = ["1", "2", "3"]
spis_vyz = main.get_university()
spis_need_olymp = ["Всерос"]
spis_olymp = main.get_olymp()
...
async def del_VYZ_2(update, context):
    ...
    if vyz in spis_vyz:
        main.user_university_delete(vyz)
        await update.message.reply_text(
...
async def add_VYZ_3(update, context):
    facul = update.message.text
    if vyz in spis_facult:
        main.user_university_add(f"{vyz}({facul})")
        await update.message.reply_text(
...
async def spic_VYZ(update, context):
    ...
    await update.message.reply_html(rf'{main.get_university()}')
...
async def spic_OLYMP(update, context):
    ...
    await update.message.reply_html(rf'{main.get_olymp()}')
